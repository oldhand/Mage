Mage_TalentMode_Hit = "我是奶骑模式"
Mage_AutoMode_Hit = "自动选择目标模式(奶骑天赋生效)"

local Mage_AutoMode = true;
local Mage_TalentMode = 0; --治疗模式
...
function Mage_TalentMode_fun()
	 if Mage_TalentMode == 0 then
		Mage_TalentMode = 1;
		Blizzard_AddMessage("**转换为防骑模式**",1,0,0,"crit");
		Mage_Default_AddMessage("**转换为防骑模式**");
		Mage_TalentMode_Hit = "防骑模式";
     ...
     elseif Mage_TalentMode == 1 then
            Mage_TalentMode = 2;
            Blizzard_AddMessage("**转换为惩戒骑模式**",1,0,0,"crit");
            Mage_Default_AddMessage("**转换为惩戒骑模式**");
            Mage_TalentMode_Hit = "惩戒骑模式";
            MageTooltip:SetText(Mage_TalentMode_Hit);
            MageTalentModeBtn:SetNormalTexture("Interface\\Icons\\Spell_Holy_AuraOfLight");
	 else
		Mage_TalentMode = 0;
		Blizzard_AddMessage("**转换为奶骑模式**",1,0,0,"crit");
	    Mage_Default_AddMessage("**转换为奶骑模式**");
	    Mage_TalentMode_Hit = "奶骑模式";
		MageTooltip:SetText(Mage_TalentMode_Hit);
...
function Mage_AutoMode_fun()
	 if Mage_AutoMode then
		Mage_AutoMode = false;
		Blizzard_AddMessage("**转换手动选择目标模式(奶骑天赋生效)**",1,0,0,"crit");
		Mage_Default_AddMessage("**转换手动选择目标模式(奶骑天赋生效)**");
		...
		Mage_AutoMode = true;
		Blizzard_AddMessage("**转换自动选择目标模式(奶骑天赋生效)**",1,0,0,"crit");
	    Mage_Default_AddMessage("**转换为自动选择目标模式(奶骑天赋生效)**");
